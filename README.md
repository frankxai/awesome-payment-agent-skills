...
- [stipend](https://github.com/stipend-sh/stipend) — non-custodial USDC wallet on Base as a **local stdio MCP server** (7 tools); the per-transaction, per-day and per-counterparty caps and the destination allowlist are enforced in `policy.py` between the decision and the signature, so the control surface is code rather than a system prompt, and it fails closed. Buyer-side x402 via EIP-3009. Python, Apache-2.0, unaudited.
